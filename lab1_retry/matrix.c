...
void MatrixCreate(MatrixCollection *collection, Matrix **matrix, char *name, size_t size, FieldInfo *element_type, int *response){
    ...
    *matrix = (Matrix *) malloc(add_memory * add_memory * sizeof(Matrix*));
    (*matrix)->data = malloc(add_memory * add_memory * sizeof(Matrix*));

    if(*matrix && (*matrix)->data){
        for (int i=0; i < size; i++){
            for (int j=0; j < size; j++){

                void *temporary_result = malloc(add_memory * add_memory * (*element_type).element_size);
                ...
                memcpy( (Matrix*)(*matrix)->data + i*size + j, temporary_result, (5*element_type->element_size));
    ...
    collection->matrices = realloc( collection->matrices, ( collection->size + 1) * add_memory * add_memory * sizeof(NamedMatrix));
...
void MatrixAutoCreate(MatrixCollection *collection, Matrix **matrix, char *name, size_t size, FieldInfo *element_type, int *response, void *max_number){
...
void MatrixAdd(Matrix **m_sum, Matrix **m_1, Matrix **m_2, int *response, MatrixCollection *collection, char *name){

    int add_memory = 100;//A_size;

    *m_sum = (Matrix *) malloc(add_memory * add_memory * sizeof(Matrix*));
    (*m_sum)->data = malloc(add_memory * add_memory * sizeof(Matrix*));
    ...
    if(*m_sum && (*m_sum)->data){
        ...
        for (int i=0; i < (*m_1)->size; i++){
            for (int j=0; j < (*m_1)->size; j++){
                ...
                void *temporary_result = malloc( sizeof((*m_1)->type_info.element_size));
                if(temporary_result == NULL){
                ...
                memcpy( (Matrix*)(*m_sum)->data + i*(*m_sum)->size + j, temporary_result, sizeof((*m_sum)->type_info.element_size));
...
void MatrixMultiply(Matrix **m_product, Matrix **m_1, Matrix **m_2, int *response, MatrixCollection *collection, char *name){

    int add_memory = 100;//A_size;

    *m_product = (Matrix *) malloc(add_memory * add_memory * sizeof(Matrix*));
    (*m_product)->data = malloc(add_memory * add_memory * sizeof(Matrix*));
    ...
    if(*m_product && (*m_product)->data){
        ...
        for (int i=0; i < (**m_1).size; i++){
            for (int j=0; j < (**m_1).size; j++){
                ...
                memcpy( (Matrix*)(*m_product)->data + i*(*m_product)->size + j, temporary_result, sizeof((*m_product)->type_info.element_size));
    ...
    if(*m_product && (*m_product)->data){
        ...
        for (int i=0; i < (*m_1)->size; i++){
            for (int j=0; j < (*m_1)->size; j++){
                ...
                void *temporary_result = malloc( sizeof((*m_1)->type_info.type_name));
                if(temporary_result == NULL){
...
void LinearCombinationInitialize(Matrix **linear_combination, Matrix **matrix){
    ...
    *linear_combination = (Matrix *) malloc(add_memory * add_memory * sizeof(Matrix*));
    (*linear_combination)->data = malloc(add_memory * add_memory * sizeof(Matrix*));
    ...
    if(*linear_combination && (*linear_combination)->data ){
        ...
        for (int i=0; i < (*linear_combination)->size; i++){
            for (int j=0; j < (*linear_combination)->size; j++){
                memset((*linear_combination)->data + i*(*linear_combination)->size + j, 0, sizeof((*linear_combination)->type_info.element_size));
            }
        }
    }
}
...
void MatrixAddLinearCombination(Matrix **m_result, int row_number, Matrix **m_1, MatrixCollection *collection, char *name, Matrix **linear_combination, void *max_number){
    ...
    *m_result = (Matrix *) malloc(add_memory * add_memory * sizeof(Matrix*));
    (*m_result)->data = malloc(add_memory * add_memory * sizeof(Matrix*));
    ...
    if(*m_result && (*m_result)->data){
        ...
        for (int i=0; i < (*m_1)->size; i++){
            ...
            if(i+1 != row_number){
                ...
                void *multiplier = malloc(add_memory * add_memory * sizeof((*m_1)->type_info.type_name));
                if(multiplier == NULL){
                ...
                for (int j=0; j < (*m_1)->size; j++){
                    ...
                    void *temporary_result_1 = malloc( sizeof((*m_1)->type_info.type_name));
                    if(temporary_result_1 == NULL){
                        printf("\nError in MatrixAddLinearCombination: Couldn't allocate memory space for temporary_result\n");
                        exit(-1);
                    }

                    void *temporary_result_2 = malloc( sizeof((*m_1)->type_info.type_name));
                    ...
                    memcpy( (Matrix*)(*linear_combination)->data + j, temporary_result_2, sizeof((*linear_combination)->type_info.element_size));

                    memcpy( (Matrix*)(*m_result)->data + i*(*m_result)->size + j, (Matrix*)(*m_1)->data + i*(*m_result)->size + j, sizeof((*m_result)->type_info.element_size));
    ...
    for (int i=0; i < (*m_1)->size; i++){
        for (int j=0; j < (*m_1)->size; j++){
            if(i+1 == row_number){
                ...
                void *result = malloc( sizeof((*m_1)->type_info.type_name));
                if(result == NULL){
                ...
                memcpy( (Matrix*)(*m_result)->data + i*(*m_result)->size + j, result, sizeof((*m_result)->type_info.element_size));
...
char *GetElementTypeName(Matrix **matrix){
    char *type_name;
    type_name = (*matrix)->type_info.type_name;
    return type_name;
}